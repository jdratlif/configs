...
def on_window(i3, e):
    ...
    try:
        ...
        verbose(
            f"Class: {e.ipc_data['container']['window_properties']['class']}, instance: {e.ipc_data['container']['window_properties']['instance']}"
        )
    ...
    if args.nocheck:
        ...
        new_window = e.ipc_data["container"]["app_id"]
        if not new_window:
            new_window = e.ipc_data["container"]["window_properties"]["instance"]
        if new_window == waitfor:
            verbose(f"a new window for '{waitfor}' appeared")
            finished = True
        else:
            verbose(
                f"a new window appeared '{new_window}' but I'm waiting for '{waitfor}'"
            )
...
http://www.ibiblio.org/pub/linux/X11/xutils/toolwait-0.9.tar.gz
    ...
    parser.add_argument(
        "-m", "--mark", dest="mark", type=str, help="mark to add to window", default=""
    )
    parser.add_argument(
        "-t",
        "--timeout",
        dest="timeout",
        type=float,
        help="timeout (default = %(default)s secs)",
        default=30.0,
    )
    parser.add_argument(
        "-w",
        "--waitfor",
        dest="waitfor",
        help="app_id (wayland) or instance string (xwayland) to wait for",
    )
